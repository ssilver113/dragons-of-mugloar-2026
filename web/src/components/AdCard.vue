...
const props = defineProps<{ ad: AdView; advisor: boolean; solving: boolean; disabled: boolean }>()
...
const warnings = computed(() =>
  props.ad.flags.flatMap((flag) => ADVISOR_FLAGS[flag] ?? []),
)
...
// thing the player is not offered. Bad odds are still the player's call to make (D61).
const unsendable = computed(() => props.ad.flags.includes('UNREADABLE'))
</script>

<template>
  <li
    class="flex flex-col gap-3 rounded-lg border border-ink-muted/20 bg-surface-raised p-4"
    :class="{ 'opacity-60': unsendable }"
  >
    ...
    <div v-if="advisor" class="rounded-md border border-accent/30 bg-surface/60 p-3">
      <p class="text-xs font-semibold uppercase tracking-wide text-accent">Advisor's read</p>
      <dl class="mt-1.5 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
        <div>
      ...
      <ul v-if="warnings.length" class="mt-2 flex flex-col gap-1">
