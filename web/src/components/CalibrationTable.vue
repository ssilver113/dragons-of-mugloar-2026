...
/**
 * The advisor's own record: what it said before each job, against how those jobs went.
 *
 * It carries no surface of its own. It was a disclosure sitting under the board on a panel of its
 * own, which put the advice and the advice's track record in two different boxes; it is the lower
 * half of the advisor's working sheet now, and the sheet it is on is the caller's. The end screen,
 * where there is no advisor and no board left, wraps it in a panel instead.
 */
...
const summary = computed(() =>
  props.attempts === 0
    ...
    : `${props.attempts} jobs across ${props.games} ${props.games === 1 ? 'game' : 'games'}.`,
)
</script>

<template>
  <section aria-labelledby="calibration-heading" class="flex flex-col gap-2">
    <h3
      id="calibration-heading"
      class="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-advisor"
    >
      <svg
        viewBox="0 0 24 24"
        ...
        focusable="false"
      >
        <path d="M4 20h16M7 20V9m5 11V4m5 16v-7" />
      </svg>
      How the advice has held up
    </h3>

    <p class="text-xs text-ink-muted">
      {{ summary }} What the advisor predicted before each job, against how those jobs actually
      went. The tally carries across games so the estimate has enough attempts to settle; a label
      needs {{ MIN_SAMPLE }} before its rate is worth reading.
    </p>
